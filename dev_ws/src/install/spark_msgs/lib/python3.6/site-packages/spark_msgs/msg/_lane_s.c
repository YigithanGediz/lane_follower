...
bool spark_msgs__msg__lane__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp(
        "spark_msgs.msg._lane.Lane",
        full_classname_dest, 25) == 0);
  }
  spark_msgs__msg__Lane * ros_message = _ros_message;
  {  // lane
    ...
    if (!rosidl_generator_c__int16__Sequence__init(&(ros_message->lane), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create int16__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    int16_t * dest = ros_message->lane.data;
...
PyObject * spark_msgs__msg__lane__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("spark_msgs.msg._lane");
  ...
  {  // lane
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "lane");
    if (!field) {
      return NULL;
    }
    ...
    if (ros_message->lane.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      int16_t * src = &(ros_message->lane.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->lane.size * sizeof(int16_t));
      assert(data != NULL);
